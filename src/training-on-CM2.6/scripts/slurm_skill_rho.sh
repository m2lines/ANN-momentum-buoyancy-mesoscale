...
singularity exec $NVFLAG --overlay /scratch/$USER/Pavel_container.ext3:ro \
    ...
    /bin/bash -c "export FACTORS='$FACTORS' SPLIT='$SPLIT' NTIME='$NTIME' DEVICE='$DEVICE' ${SUBFILTER:+SUBFILTER='$SUBFILTER'} ${ANN_PATH:+ANN_PATH='$ANN_PATH'} ${SKILL_OUT:+SKILL_OUT='$SKILL_OUT'}; source /ext3/env.sh; time python -u compute_skill_rho.py"
