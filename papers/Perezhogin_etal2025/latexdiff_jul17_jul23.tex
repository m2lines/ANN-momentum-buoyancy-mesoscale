...
To facilitate generalization across different resolutions \cite<scale-aware or grid-aware parameterization,>{bachman2017scale}, we account for the local grid spacing of the coarse resolution model $\Delta = \sqrt{\Delta x \Delta y}$, resulting in the following \DIFaddbegin \DIFadd{functional form of the }\DIFaddend parameterization \cite{lund1993parameterization, li2025transformer}:
\begin{equation}
    \mathbf{T} \approx \widehat{\mathbf{T}}(\mathbf{X}, \Delta). \label{eq:model_2}
\end{equation}
\DIFdelbegin \DIFdel{Including }\DIFdelend \DIFaddbegin \DIFadd{Accounting for }\DIFaddend grid spacing is physically justified as velocity gradients and momentum fluxes differ in dimensionality and require a length scale to be invoked.

%

\subsection{Neural Network Parameterizations} \label{sec: eddy_param}

We consider a baseline data-driven parameterization of eddy fluxes with fixed normalization coefficients\DIFaddbegin \DIFadd{, following the form of Eq. \eqref{eq:model_2}}\DIFaddend :
\begin{equation}
...
\section{\DIFdelbegin \DIFdel{Training dataset, ANN architecture and experimental }\DIFdelend \DIFaddbegin \DIFadd{Experimental }\DIFaddend setups}

\DIFaddbegin \subsection{\DIFadd{Training dataset}}
\DIFaddend The training dataset is created using the climate model CM2.6 \cite{griffies2015impacts}, which has a nominal ocean resolution of $0.1^\circ$.
Velocity gradients (Eq. \eqref{eq:ZB_features})\DIFaddbegin \DIFadd{, }\DIFaddend used as input features\DIFaddbegin \DIFadd{, }\DIFaddend and subfilter forcing ($\mathbf{S}$, Eq. \eqref{eq:subfilter_forcing})\DIFaddbegin \DIFadd{, }\DIFaddend used as output\DIFaddbegin \DIFadd{, }\DIFaddend are diagnosed using horizontal filtering \DIFdelbegin \DIFdel{and then }\DIFdelend \DIFaddbegin \DIFadd{followed by }\DIFaddend coarse-graining, \DIFdelbegin \DIFdel{to avoid }\DIFdelend \DIFaddbegin \DIFadd{which avoids the }\DIFaddend inclusion of discretization errors \cite{guillaumin2021stochastic,christensen2022parametrization,agdestein2025discretize}.
...
\subsection{\DIFadd{ANN architecture}}
\DIFaddend For the offline analysis of parameterizations (Eqs. \eqref{eq:model_1}, \eqref{eq:model_3}), we found that an ANN with two hidden layers, 32 neurons each, was sufficiently large to effectively learn from the input features. See Text S3 in SI for details. 

\DIFdelbegin \DIFdel{For the online evaluation, we }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Online implementation}} \label{sec:online_implementation}
\DIFadd{We }\DIFaddend implement the ANN mesoscale eddy parameterization \DIFaddbegin \DIFadd{(Eq. \eqref{eq:model_3}) }\DIFaddend in two considerably different configurations of the GFDL MOM6 ocean model \cite{adcroft2019gfdl} \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{at eddy-permitting ($1/4^\circ$) resolution. To ensure that ANN inference remains computationally efficient, we retrain a smaller network with only one hidden layer and 20 neurons, which keeps the ANN inference time below 10\% of the ocean model runtime (Text S3 in SI for details). While our goal was to implement the ANN parameterization without further modifications, minor adjustments were necessary for numerical stability, see Text S4 in the SI.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Note that we retrained a smaller ANN with only one hidden layer and 20 neurons, so that the ANN inference time remains within 10\% of the ocean model runtime (Text S3 in SI for details). In addition, while we aimed to implement the ANN parameterization without further modifications, some adjustments were necessary for numerical stability, see Text S4 in the SI.
}%DIFDELCMD < 
...
We \DIFdelbegin \DIFdel{implement ANN with the local dimensional scaling (Eq.~\eqref{eq:model_3}) using only one hidden layer with 20 neurons to constrain the inference time. Additionally, we use all }\DIFdelend \DIFaddbegin \DIFadd{use all available }\DIFaddend depths and grid resolutions shown in Figure \ref{fig:offline}(d) for training to make \DIFdelbegin \DIFdel{ANN parameterization }\DIFdelend \DIFaddbegin \DIFadd{the implemented ANN parameterization (Eq. \eqref{eq:model_3}) }\DIFaddend less tied to any specific resolution or depth. The retrained, smaller ANN \DIFaddbegin \DIFadd{(see Section \ref{sec:online_implementation}) }\DIFaddend exhibits a slightly lower offline skill ($R^2$) than the version described earlier, on average, by 0.1 (Figure S5 in SI).
...
\DIFdel{(archived at }%DIFDELCMD < \url{https://doi.org/10.5281/zenodo.15989128}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeA{perezhogin_2025_software}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The training dataset, offline skill, and simulation data are available at \DIFdelbegin %DIFDELCMD < \url{https://doi.org/10.5281/zenodo.15990039}%%%
...
This project is supported by Schmidt Sciences, LLC\DIFaddbegin \DIFadd{. 
LZ was also partially funded through NOAA NA19OAR4310364-T1-01}\DIFaddend . 
